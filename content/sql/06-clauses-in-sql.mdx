...
In SQL (Structured Query Language), clauses are used to filter, group, and sort the data retrieved from a database. Clauses are used in conjunction with SQL commands to perform specific operations on the data. Some common clauses used in SQL queries include:
...
```sql
SELECT emp_dept, AVG(emp_salary) FROM employees GROUP BY emp_dept HAVING AVG(emp_salary) > 50000;
```
