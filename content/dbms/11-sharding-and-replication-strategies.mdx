...
Distributed databases are databases stored on multiple computers or servers connected to each other. Each server has its own database and can perform operations independently.

Distributed databases can be sharded or replicated to improve performance, availability, and fault tolerance.

They can store and manage large volumes of data, support high transaction throughput, and provide real-time access to critical data.
...
A distributed database system can only provide two out of the three guarantees:

- **Consistency**: Every read receives the most recent write or an error.
- **Availability**: Every request receives a response, without a guarantee that it contains the most recent write.
- **Partition Tolerance**: The system continues to operate despite network partitions.

When designing a distributed database system, trade-offs must be made between consistency, availability, and partition tolerance based on application requirements.
...
- Use **distributed databases** when you need to store and manage large volumes of data, support high transaction throughput, and provide real-time access to critical data.